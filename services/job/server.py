"""
Job service gRPC server bootstrap (skeleton phase).

Responsibilities in this phase:
- load job config
- initialize logging
- register JobInternalService servicer
...
#   - python -m services.job.server
#   - python services/job/server.py
# -----------------------------------------------------------------------------
...
try:
    from . import servicer as job_servicer_module
except ImportError:
    import servicer as job_servicer_module  # type: ignore
...
def _coerce_int(value: Any, default: int) -> int:
    """Parse integer configuration values for job server startup."""
    try:
        return int(value)
    except Exception:
        return default
...
def _resolve_job_config() -> Any:
    """
    Supports multiple common.load_service_config signatures:
      - load_service_config("job")
      - load_service_config(service_name="job")
      - load_service_config()
    ...
    if load_service_config is not None:
        ...
        try:
            return load_service_config("job")
        except TypeError:
            pass
        except Exception:
            pass
        ...
        # Try keyword.
        try:
            return load_service_config(service_name="job")
        except TypeError:
            pass
        except Exception:
            pass
...
def _resolve_logger(cfg: Any) -> logging.Logger:
    """Create the job service logger from config and environment settings."""
    service_name = getattr(cfg, "service_name", "job")
...
def _resolve_servicer_class() -> Type[Any]:
    ...
    candidates = [
        "JobServicer",
        "JobInternalServicer",
        "JobInternalServiceServicerImpl",
    ]
    for name in candidates:
        cls = getattr(job_servicer_module, name, None)
        if inspect.isclass(cls):
            return cls  # type: ignore[return-value]

    raise RuntimeError(
        "Could not find a Job servicer class in services/job/servicer.py. "
...
def _bind_addr(cfg: Any) -> str:
    """Return the job gRPC server bind address as "host:port"."""
    host = getattr(cfg, "bind_host", None) or getattr(cfg, "host", "0.0.0.0")
    port = _coerce_int(getattr(cfg, "port", getattr(cfg, "job_port", 50052)), 50052)
    return f"{host}:{port}"


def _serve() -> None:
    """
    Starts the Job gRPC server and blocks until shutdown signal.
    """
    cfg = _resolve_job_config()
    ...
    if bound_port == 0:
        raise RuntimeError(f"Failed to bind Job gRPC server to '{bind_addr}'")
    ...
    _emit(
        ...
        service=service_name,
        bind_addr=bind_addr,
        grpc_service="taskqueue.internal.v1.JobInternalService",
    )
