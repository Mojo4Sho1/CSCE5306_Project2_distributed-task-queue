...
def _ensure_import_paths() -> None:
    """Ensure local import paths include repo and generated modules."""
...
def _build_stubs(public_pb2_grpc, targets: Sequence[str]) -> List[object]:
    """Build gRPC stubs used by smoke checks."""
...
def _other_index(owner_index: int, node_count: int) -> int:
    """Return the alternate node index used in routing checks."""
    if node_count <= 1:
        return owner_index
