#!/usr/bin/env python3
"""
Smoke test for Job skeleton server.

What this validates:
1) Job process starts and stays alive.
2) gRPC endpoint becomes reachable.
3) All 6 Job internal RPCs are registered and callable.
4) Each RPC returns UNIMPLEMENTED (skeleton-phase expected behavior).
5) Job service shuts down cleanly after test.

Usage:
    python scripts/smoke_job_skeleton.py
    python scripts/smoke_job_skeleton.py --host 127.0.0.1 --port 50052
"""
...
def _repo_root() -> Path:
    # scripts/smoke_job_skeleton.py -> repo root is parent of scripts
...
def _print_summary_and_exit(checks: List[CheckResult]) -> int:
    print("\n=== Job Skeleton Smoke Test Summary ===")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Job skeleton smoke test")
    parser.add_argument("--host", default="127.0.0.1", help="Job bind host to probe")
    parser.add_argument("--port", type=int, default=50052, help="Job bind port to probe")
    parser.add_argument(
        "--startup-timeout",
        type=float,
        default=15.0,
    ...
    try:
        ...
        checks.append(
            CheckResult(
                name="process_start",
                passed=alive,
                detail="job process is running" if alive else f"exited early with code {proc.returncode}",
            )
        )
        if not alive:
        ...
        with grpc.insecure_channel(addr) as channel:
            ...
            rpc_cases = [
                ...
                (
                    "DeleteJobIfStatus",
                    lambda: stub.DeleteJobIfStatus(
                        pb2.DeleteJobIfStatusRequest(job_id="smoke-job", expected_status=0),
                        timeout=args.rpc_timeout,
                    ),
                ),
                (
                    "GetJobRecord",
                    lambda: stub.GetJobRecord(
                        pb2.GetJobRecordRequest(job_id="smoke-job"),
                        timeout=args.rpc_timeout,
                    ),
                ),
                (
                ...
                (
                    "TransitionJobStatus",
                    lambda: stub.TransitionJobStatus(
                        pb2.TransitionJobStatusRequest(
                            job_id="smoke-job",
                            expected_from_status=0,
                            to_status=0,
                            actor="smoke",
                            reason="smoke",
                        ),
                ...
                (
                    "SetCancelRequested",
                    lambda: stub.SetCancelRequested(
                        pb2.SetCancelRequestedRequest(
                            job_id="smoke-job",
        ...
        checks.append(
            CheckResult(
                ...
                passed=still_alive,
                detail="job still running after RPC probes"
                if still_alive
                else f"job exited unexpectedly with code {proc.returncode}",
            )
        )
