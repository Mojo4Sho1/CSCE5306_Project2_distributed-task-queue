#!/usr/bin/env python3
"""
Smoke test for Queue skeleton server.

What this validates:
1) Queue process starts and stays alive.
2) gRPC endpoint becomes reachable.
3) All 3 Queue internal RPCs are registered and callable.
4) Each RPC returns UNIMPLEMENTED (skeleton-phase expected behavior).
5) Queue service shuts down cleanly after test.

Usage:
    python scripts/smoke_queue_skeleton.py
    python scripts/smoke_queue_skeleton.py --host 127.0.0.1 --port 50053
"""
...
def _repo_root() -> Path:
    # scripts/smoke_queue_skeleton.py -> repo root is parent of scripts
...
def _print_summary_and_exit(checks: List[CheckResult]) -> int:
    print("\n=== Queue Skeleton Smoke Test Summary ===")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Queue skeleton smoke test")
    parser.add_argument("--host", default="127.0.0.1", help="Queue bind host to probe")
    parser.add_argument("--port", type=int, default=50053, help="Queue bind port to probe")
    parser.add_argument(
        "--startup-timeout",
        type=float,
        default=15.0,
    ...
    try:
        ...
        checks.append(
            CheckResult(
                name="process_start",
                passed=alive,
                detail="queue process is running" if alive else f"exited early with code {proc.returncode}",
            )
        )
        if not alive:
            ...
            rpc_cases = [
                (
                    ...
                    lambda: stub.EnqueueJob(
                        pb2.EnqueueJobRequest(job_id="smoke-job", enqueued_at_ms=0),
                        timeout=args.rpc_timeout,
                    ),
                ),
                (
                    "DequeueJob",
                    lambda: stub.DequeueJob(
                        pb2.DequeueJobRequest(worker_id="smoke-worker"),
                        timeout=args.rpc_timeout,
                    ),
                ),
                (
                    ...
                    lambda: stub.RemoveJobIfPresent(
                        pb2.RemoveJobIfPresentRequest(job_id="smoke-job"),
                        timeout=args.rpc_timeout,
                    ),
                ),
            ]
        ...
        checks.append(
            CheckResult(
                ...
                passed=still_alive,
                detail="queue still running after RPC probes"
                if still_alive
                else f"queue exited unexpectedly with code {proc.returncode}",
            )
        )
