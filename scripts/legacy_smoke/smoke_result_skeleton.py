#!/usr/bin/env python3
"""
Smoke test for Result skeleton server.

What this validates:
1) Result process starts and stays alive.
2) gRPC endpoint becomes reachable.
3) All 2 Result internal RPCs are registered and callable.
4) Each RPC returns UNIMPLEMENTED (skeleton-phase expected behavior).
5) Result service shuts down cleanly after test.

Usage:
    python scripts/smoke_result_skeleton.py
    python scripts/smoke_result_skeleton.py --host 127.0.0.1 --port 50055
"""
...
def _repo_root() -> Path:
    # scripts/smoke_result_skeleton.py -> repo root is parent of scripts
...
def _print_summary_and_exit(checks: List[CheckResult]) -> int:
    print("\n=== Result Skeleton Smoke Test Summary ===")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Result skeleton smoke test")
    parser.add_argument("--host", default="127.0.0.1", help="Result bind host to probe")
    parser.add_argument("--port", type=int, default=50055, help="Result bind port to probe")
    parser.add_argument(
        "--startup-timeout",
        type=float,
        default=15.0,
    ...
    try:
        ...
        checks.append(
            CheckResult(
                name="process_start",
                passed=alive,
                detail="result process is running" if alive else f"exited early with code {proc.returncode}",
            )
        )
        if not alive:
        ...
        with grpc.insecure_channel(addr) as channel:
            stub = pb2_grpc.ResultInternalServiceStub(channel)
            ...
            rpc_cases = [
                ...
                (
                    ...
                    lambda: stub.GetResult(
                        pb2.GetResultRequest(job_id="smoke-job"),
                        timeout=args.rpc_timeout,
                    ),
                ),
            ]
        ...
        checks.append(
            CheckResult(
                ...
                passed=still_alive,
                detail="result still running after RPC probes"
                if still_alive
                else f"result exited unexpectedly with code {proc.returncode}",
            )
        )
