...
NFR-6 & \textbf{Capacity Framing}: Comparisons lock node count and workload controls, while reporting realized as-run worker-loop capacity separately. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Capacity interpretation:} the recorded starter-matrix runs used equal node count but different as-run worker-loop capacity (Design A: 1 loop, Design B: 6 loops).
...
That interpretation is consistent with your intuition: the recorded runs used equal node count but different as-run worker-loop capacity (Design A: 1 loop, Design B: 6 loops).
...
\noindent
Interpretation must retain fairness guardrails from \texttt{docs/spec/fairness-evaluation.md}: equal node count with explicit as-run worker-loop disclosure (A=1, B=6 for starter-matrix evidence), Design B owner-routing rules for job-scoped operations, fixed timing windows, and \texttt{ListJobs=0\%} in primary parity scenarios.
